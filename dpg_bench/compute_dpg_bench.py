...
LOG_PATH = "./dpg_bench/org_eval_error_log.txt"
...
def main():
    ...
    if args.res_path is None:
        args.res_path = osp.join(args.image_root_path, f'{time_style}_org_results.txt')
    ...
    for fn in tqdm(local_filename_list):
        ...
        try:
            ...
            with open(LOG_PATH, "a", encoding="utf-8") as log_file:
                log_file.write(f"[{timestamp}] Episode #{fn} failed:\n")
                log_file.write(f"{str(e)}\n")
                log_file.write(tb + "\n")
                log_file.write("-" * 80 + "\n")

            # 경고는 콘솔에도 출력
            print(f"[Warning] episode #{fn} failed, logged to {LOG_PATH}")
