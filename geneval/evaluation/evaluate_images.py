...
LOG_PATH = "./geneval/org_eval_error_log.txt"

# Get directory path

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--imagedir", type=str, default="./geneval/com_samples3")
    parser.add_argument("--outfile", type=str, default="./geneval/com_results3.jsonl")
    ...
    if args.model_config is None:
        print(os.path.dirname(mmdet.__file__))
...
def load_models(args):
    ...
    CKPT_PATH = os.path.join(args.model_path, f"geneval/mmdetection/configs/mask2former/{OBJECT_DETECTOR}.pth")
    print(CKPT_PATH)
...
def main(args):
    ...
    eval_contents = []
    with open(os.path.join(prompt_dir, "evaluation_metadata.jsonl"), "r", encoding="utf-8") as f:
        eval_text = f.read().splitlines()
    eval_contents = [(i, line) for i, line in enumerate(eval_text, start=1)]

    full_results = []
    idx = 0
    for img_file in os.listdir(args.imagedir):
        # print("img_file : ", img_file)
        try:
            print("episode : ", idx+1, "/", len(eval_contents))  
            image_path = os.path.join(args.imagedir, img_file)
            # print("image_path : ", image_path)
            # if not os.path.isdir(image_path) or not img_file.isdigit():
            #     continue
            metadata = eval_contents[idx][1]
            # print(type(metadata), metadata)
            if isinstance(metadata, str):
                metadata = json.loads(metadata)

            # Evaluate each image
            result = evaluate_image(image_path, metadata)
            # print("result : ", result)
            ...
            with open(LOG_PATH, "a", encoding="utf-8") as log_file:
                log_file.write(f"[{timestamp}] Episode #{idx} failed:\n")
                log_file.write(f"{str(e)}\n")
                log_file.write(tb + "\n")
                log_file.write("-" * 80 + "\n")

            # 경고는 콘솔에도 출력
            print(f"[Warning] episode #{idx+1} failed, logged to {LOG_PATH}")
            idx += 1
            continue
    # print("full result : ", full_results)
